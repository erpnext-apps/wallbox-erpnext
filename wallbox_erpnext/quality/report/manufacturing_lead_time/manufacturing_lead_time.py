...
def get_data(filters):
	...
	so_details = frappe.db.sql("""select so.customer, so.name, so.transaction_date,
		soi.item_code, soi.item_name, soi.item_group, so.delivery_status,
		...
		join `tabWork Order` wo on (soi.item_code=wo.production_item and 
