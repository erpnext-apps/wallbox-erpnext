	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			report_data = [item, item_map[item]["item_name"],
				...
				qty_dict.out_val, qty_dict.bal_qty, qty_dict.booked_qty,
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		...
		where sle.docstatus < 2 and sle.serial_no IS NOT NULL %s %s
...
def get_item_warehouse_map(filters, sle):
	...
	iwb_map = update_so_booked_stock(iwb_map, from_date, to_date)

	return iwb_map

def update_so_booked_stock(iwb_map, from_date, to_date):
	for key in sorted(iwb_map):
		booked_qty = frappe.db.sql("""select sum(sales_order IS NOT NULL)
		from `tabSerial No` where delivery_date IS NULL and company=%s and
		item_code=%s and warehouse=%s  and purchase_date between %s and %s
		group by item_code""", (key[0], key[1], key[2], from_date, to_date))
		if booked_qty and booked_qty[0]:
			iwb_map[key]["booked_qty"] = booked_qty[0][0]
	return iwb_map
